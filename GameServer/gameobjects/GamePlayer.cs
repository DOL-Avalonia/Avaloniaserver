...
namespace DOL.GS
{
    ...
    public class GamePlayer : GameLiving, IGamePlayer
	{
		...
		{
			AbstractGameKeep keep = GameServer.KeepManager.GetKeepCloseToSpot(this.CurrentRegionID, this.Position, WorldMgr.VISIBILITY_DISTANCE(CurrentRegion));
			...
			if (bound)
			{
				if (!IsMoving)
				{
					...
					foreach (GamePlayer player in GetPlayersInRadius(WorldMgr.VISIBILITY_DISTANCE(CurrentRegion)))
					{
						...
						if ((int)player.Client.Version < (int)GameClient.eClientVersion.Version1125)
		...
		public override void GainExperience(eXPSource xpSource, long expTotal, long expCampBonus, long expGroupBonus, long expOutpostBonus, bool sendMessage, bool allowMultiply, bool notify, int eventMultiplicator)
		{
			...
			if (allowMultiply)
			{
				...
				if (Properties.ENABLE_ZONE_BONUSES)
				{
					...
					if (zoneBonus > 0)
					{
						Out.SendMessage(ZoneBonus.GetBonusMessage(this, (int)(zoneBonus * Properties.XP_RATE), ZoneBonus.eZoneBonusType.XP),
						                eChatType.CT_Important, eChatLoc.CL_SystemWindow);
		...
		protected override AttackData MakeAttack(GameObject target, InventoryItem weapon, Style style, double effectiveness, int interruptDuration, bool dualWield, bool ignoreLOS)
		{
			...
			switch (ad.AttackResult)
			{
				...
				case eAttackResult.Parried:
					...
					//Camouflage - Camouflage will be disabled only when attacking a GamePlayer or ControlledNPC of a GameObjects.GamePlayer.
				...
				case eAttackResult.HitUnstyled:
					{
						...
						{
							//GameObjects.GamePlayer.Attack.Location.Feet:	feet
		...
		public override void Die(GameObject killer)
		{
			...
			// GameObjects.GamePlayer.Die.CorpseLies:		{0} just died. {1} corpse lies on the ground.
		...
		public virtual void GainChampionExperience(long experience, eXPSource source)
		{
			...
			{
				double modifier = Properties.CL_XP_RATE;
			...
			System.Globalization.NumberFormatInfo format = System.Globalization.NumberFormatInfo.InvariantInfo;
			Out.SendMessage("Vous gagnez " + experience.ToString("F0", format) + " points d'experience champion", eChatType.CT_Important, eChatLoc.CL_SystemWindow);
