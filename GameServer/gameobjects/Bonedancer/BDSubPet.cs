			...
			get
			{
				...
				if (m_PetSpecLine == null && Owner is CommanderPet commander && commander.Owner is GamePlayer player)
				{
					...
					if (dbSummoningSpell != null)
					{
						...
						DBLineXSpell dbLineSpell = GameServer.Database.SelectObjects<DBLineXSpell>("SpellID=@SpellID", new QueryParameter("@SpellID", dbSummoningSpell.SpellID))?.FirstOrDefault();
						if (dbLineSpell != null)
						{
