			...
			lock (living)
			{
				...
				else
				{
					...
					if (player != null)
					{
						if (absorb > 0)
						{
							player.Out.SendMessage("Une barrière absorbe " + absorb + " dommages!", eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
							if (living is GamePlayer pl) pl.Out.SendMessage("Une barrière absorbe " + absorb + " dommages!", eChatType.CT_YouHit, eChatLoc.CL_SystemWindow);
						}
					}
