    ...
    public class GameKeepComponent : GameLiving, IComparable, IGameKeepComponent
    {
        ...
        public bool Climbing
        {
            get
            {
                ...
                {
                    if (m_skin == (int)eComponentSkin.Wall || m_skin == (int)eComponentSkin.NewSkinClimbingWall || m_skin == (int)eComponentSkin.Tower || m_skin == (int)eComponentSkin.NewSkinTower && !Keep.IsPortalKeep)
                    {
                        return true;
                    }
                }
                ...
                {
                    if (m_skin == (int)eComponentSkin.Wall || m_skin == (int)eComponentSkin.NewSkinClimbingWall && !Keep.IsPortalKeep)
                    {
                        return true;
                    }
                }
        ...
        public virtual void FillPositions()
        {
                    ...
                    if (positionGroup[i] is DBKeepPosition position)
                    {
                        ...
                        else
                        {
							...
							* Otherwise, we're assuming the former position/component combination wasn't valid, and that's an error that should be reported in any case.
                            ...
                            {
                                case "DOL.GS.Keeps.GameKeepBanner":
									if (Keep.Banners[position.TemplateID] is IKeepItem banner && banner.Position != position)
								...
								default:
									if (Keep.Guards[position.TemplateID] is IKeepItem guard)
            ...
            foreach (GameKeepBanner banner in Keep.Banners.Values)
            {
