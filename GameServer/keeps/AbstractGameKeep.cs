...
namespace DOL.GS.Keeps
{
	...
	public abstract class AbstractGameKeep : IGameKeep
	{
		...
		{
			Guards = new Dictionary<string, GameKeepGuard>();
			KeepComponents = new List<GameKeepComponent>();
			Banners = new Dictionary<string, GameKeepBanner>();
			Doors = new Dictionary<string, GameKeepDoor>();
			Patrols = new Dictionary<string, Patrol>();
		}
		...
		{			
			...
			if (DBKeep.ClaimedGuildName != null && DBKeep.ClaimedGuildName != "")
			{
				...
				{
					this.Guild = myguild;
					this.Guild.ClaimedKeeps.Add(this);
					StartDeductionTimer();
				}
			}
		...
		public virtual bool CheckForClaim(GamePlayer player)
		{
			...
			if (player.Group != null)
			{
				...
				{
					if (GameServer.KeepManager.GetKeepCloseToSpot(p.CurrentRegionID, p.Position, WorldMgr.VISIBILITY_DISTANCE(p.CurrentRegion)) == this)
						count++;
				}
