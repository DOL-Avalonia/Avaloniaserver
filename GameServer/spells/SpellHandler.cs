...
namespace DOL.GS.Spells
{
	...
	public class SpellHandler : ISpellHandler
	{
		...
		public virtual void FinishSpellCast(GameLiving target)
		{
			...
			{
				MessageToCaster(LanguageMgr.GetTranslation((Caster as GamePlayer)?.Client.Account.Language ?? "FR", "RealmAbility.SendCasterSpellEffectAndCastMessage.You", Spell.Name), eChatType.CT_Spell);
				foreach (GamePlayer player in m_caster.GetPlayersInRadius(WorldMgr.INFO_DISTANCE))
				{
					if (player != m_caster)
						player.MessageFromArea(m_caster, LanguageMgr.GetTranslation(player.Client.Account.Language, "GameObject.Casting.CastsASpell", Caster.GetName(0, true)), eChatType.CT_Spell, eChatLoc.CL_SystemWindow);
				}
			}
