...
namespace GameServerScripts.Amtescripts.Managers
{
    public class DeathCheck
    {
        ...
        {
            var deaths = GameServer.Database.SelectObjects<DBDeathLog>("KilledId = @killed AND isWanted = 0 AND DeathDate < SUBTIME(NOW(), '12:0:0')", new QueryParameter("killed", player.InternalID));
            ...
            {
                var client =  WorldMgr.GetClientByPlayerName(death.Killer, true, true);
